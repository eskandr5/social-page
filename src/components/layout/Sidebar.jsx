  ...
  return (
    <div className="hidden md:flex flex-col h-[calc(100vh-100px)] sticky top-24 p-2 justify-between">
        ...
        <nav className="space-y-2">
          {menuItems.map((item, index) => {
            ...
            const baseClass = disabled
              ? "flex items-center gap-4 p-3 rounded-xl transition-all duration-200 cursor-not-allowed text-slate-400 bg-slate-100/40"
            ...
            const content = (
              <>
                <span
                  className={`${active ? "" : "group-hover:scale-110"} transition-transform ${disabled ? "text-slate-400" : ""}`}
                >
                  {item.icon}
                </span>
                <span
                  className={`font-bold hidden lg:block ${disabled ? "text-slate-400" : active ? "text-white" : "text-gray-700 group-hover:text-blue-600"}`}
                >
                  {item.label}
                </span>
              </>
            ...
            return (
              <Link key={index} to={item.path} className={baseClass}>
                {content}
              </Link>
            );
          })}
      ...
      >
        ...
        />
        <span className="font-bold hidden lg:block text-gray-700 group-hover:text-red-600">
          Logout
        </span>
      </button>
    </div>
