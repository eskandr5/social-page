...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-200 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-8 text-gray-500">
            <Link
              to="/feed"
              className={`transition-colors cursor-pointer ${isActive("/feed") ? "text-blue-600" : "hover:text-blue-600"}`}
            >
              <AiOutlineHome size={26} />
            </Link>
            <Link
              to="/notifications"
              className={`relative transition-colors cursor-pointer ${isActive("/notifications") ? "text-blue-600" : "hover:text-blue-600"}`}
            >
            ...
            <div
              className="flex items-center cursor-not-allowed text-slate-400 transition-colors"
              title="Messenger is temporarily disabled"
            >
          ...
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="hidden sm:flex items-center gap-2 text-gray-500 hover:text-red-600 transition-colors font-bold text-sm cursor-pointer"
            >
      ...
      {/* Overlay */}
      <div
        className={`fixed inset-0 bg-gray-900/40 backdrop-blur-sm z-60 md:hidden transition-opacity duration-300 ${
      ...
      {/* Sidebar Content */}
      <div
        className={`fixed top-0 left-0 h-full w-72 bg-white z-70 shadow-2xl transform transition-transform duration-300 ease-out md:hidden flex flex-col ${
